...
class Net:
    ...
    def __pre_processing(self):
        ...
        train_generator = train_datagen.flow_from_directory(self.train_dir,target_size=(150, 150),batch_size=30,class_mode='binary')
        validation_generator = test_datagen.flow_from_directory(self.validation_dir,target_size=(150, 150),batch_size=30,class_mode='binary')
        test_generator = test_datagen.flow_from_directory(self.test_dir,target_size=(150,150),batch_size=30,class_mode=None)
        print("pre_processing")
        return train_generator,test_generator,validation_generator

    def train(self,model,train_generator,validation_generator,epochs=30, batch_size=64, model_name = 'cat-dog-classifier'):
        model.fit_generator(train_generator,steps_per_epoch=60,epochs=1,
                              validation_data=validation_generator,validation_steps=50)
        print("train")
        return model
        
    def evaluate(self,model,validation_generator,test_generator):
    ...
    def build_model(self):
        ...
        if self.name == 'OrgNet':
            return self.__OrgNet_model()
        if self.name == 'LeNet':
            return self.__LeNet_model()
        elif self.name == 'MINST':
            return self.__MINST_model()
        elif self.name == 'CIFAR10':
            return self.__CIFAR10_model()
        elif self.name == 'AlexNet':
            return self.__AlexNet_model()
        elif self.name == 'VGGNet':
            return self.__VGGNet_model()
        elif self.name == 'GoogLeNet':
            return self.__GoogLeNet_model()
        elif self.name == 'ResNet':
            return self.__ResNet_model()
            ...
            print(self.name)
            return self.__CoapNet_model()

    def __AlexNet_model(self):
        inputsize = self.input_width * self.input_height * self.input_channels
        #print("Inputlayer-size: %d" % (inputsize))
        alexnet = Sequential()
        # Layer 1
        alexnet.add(Conv2D(96, (11, 11), input_shape=inputsize,padding='same'))
    ...
    def __LeNet_model(self):
        inputsize = self.input_width * self.input_height * self.input_channels
        LeNet = Sequential()
        LeNet.add(Conv2D(6,(5,5),activation='relu',input_shape=inputsize,
                         kernel_regularizer=l2(0.01)))
        LeNet.add(MaxPooling2D(pool_size=(2, 2)))
        LeNet.add(Conv2D(16,(5,5),activation='relu',input_shape=inputsize,
                          kernel_regularizer=l2(0.01)))
        LeNet.add(MaxPooling2D(pool_size=(2, 2)))
        LeNet.add(Flatten())
        LeNet.add(Dense(120))
    ...
    def __MNIST_model(self):
        inputsize = self.input_width * self.input_height * self.input_channels
        MNIST = Sequential()
        MNIST.add(Conv2D(32,(3,3),activation='relu',input_shape=inputsize, kernel_regularizer=l2(0.01)))
    ...
    def __OrgNet_model(self):
        inputsize = self.input_width * self.input_height * self.input_channels
        OrgNet = Sequential()
        OrgNet.add(Conv2D(32,(3,3),activation='relu',input_shape=inputsize))
    ...
    def __CoapNet_model(self):
        inputsize = self.input_width * self.input_height * self.input_channels
        CoapNet = Sequential()
        CoapNet.add(Conv2D(64,(3,3),activation='relu',input_shape=inputsize))
    ...
    def __CIFAR10_model(self):
        inputsize = self.input_width * self.input_height * self.input_channels
        CIFAR10 = Sequential()
        CIFAR10.add(Conv2D(32,(3,3),activation='relu',input_shape=inputsize))
    ...
    def __PlankNet_model(self):
        inputsize = self.input_width * self.input_height * self.input_channels
        PlankNet = Sequential()
        PlankNet.add(Conv2D(96,(13,13),strides=4,activation='relu',input_shape=inputsize))
    ...
    def __ResNet_model(self):
        ...
        base_model = applications.resnet50.ResNet50(weights= None, include_top=False, input_shape= inputsize)
        x = base_model.output
