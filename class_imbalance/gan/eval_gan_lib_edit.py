        ...
        with tf.Session() as sess:
          ...
          def sample_from_generator():
            ...
            if "labels" in generator.get_input_info_dict():
              ...
              #labels = tf.random.uniform([batch_size], maxval=dataset.num_classes, dtype=tf.int32)
          ...
          for i in range(num_averaging_runs):
            ...
            module_name = module_spec.split("/")[-1]
            save_dir = "results_pysil"
            photos_dir = "{}/photos".format(save_dir)
            if not os.path.exists(photos_dir):
                os.makedirs(photos_dir)
                print("\n\n Made dir: {} \n\n".format(photos_dir))

            model_dir = "{}/label{}".format(photos_dir, label)
            if not os.path.exists(model_dir):
                os.makedirs(model_dir)
                print("\n\n Made dir: {} \n\n".format(model_dir))
            else:
                print("Did not make the model dir: {}".format(model_dir))
            print(len(images))
            for j in range(len(images)):
                if j < 150:
                    plt.imshow(images[j].astype('uint8'))
                    plt.axis("off")
                    print('{}/dataset{}_image{}.png'.format(model_dir, i+1, j+1))
                    plt.savefig('{}/dataset{}_image{}.png'.format(model_dir, i+1, j+1), bbox_inches='tight', pad_inches=0, figsize=(32,32))
                    #plt.save('{}/dataset{}_image{}.tiff'.format(model_dir, i+1, j+1))
