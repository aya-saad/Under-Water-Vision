...
class ImageDatasetV2(object):
  ...
  def _parse_fn(self, features):
    print("\n\n Fra _parse_fn. Features: {} \n\n".format(features))
    image = tf.cast(features["image"], tf.float32) / 255.0 # This doesn't work.
    print("_parse_fn in _load_dataset successful \n\n\n")
...
class Whoi(ImageDatasetV2):
    """Wrapper for the WHOI dataset from homemade"""

    def __init__(self, seed):
        super(Whoi, self).__init__(
            name="whoi",
            tfds_name="whoi",
            resolution=32,
            colors=1,
            num_classes=26,
            eval_test_samples=2584,
            seed=seed)
            
class Pysil(ImageDatasetV2):
"""Wrapper for the Pysil dataset from homemade"""
